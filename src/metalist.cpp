...
void APar_Print_iTunesData(const char *path, char* output_path, uint8_t supplemental_info, uint8_t target_information, AtomicInfo* ilstAtom) {
	...
	if (supplemental_info != 0) {
		...
		if (supplemental_info & PRINT_FREE_SPACE) {
			fprintf(stdout, "free atom space: %u\n", APar_ProvideTallyForAtom("free") );
		}
		if (supplemental_info & PRINT_PADDING_SPACE) {
		...
		if ((supplemental_info & PRINT_USER_DATA_SPACE) && dynUpd.moov_udta_atom != NULL) {
			fprintf(stdout, "user data space: %" PRIu64 "\n", dynUpd.moov_udta_atom->AtomicLength);
		}
		if (supplemental_info & PRINT_MEDIA_SPACE) {
